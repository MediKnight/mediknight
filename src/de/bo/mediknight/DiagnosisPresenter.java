...
public class DiagnosisPresenter implements Presenter, Commitable, Observer {
        ...
        d.run(new Runnable() {
            public void run() {
                try {
                    ...
                    FOPrinter fop = new FOPrinter(props
                            .getProperty("diagnosis.xml"), props
                            .getProperty("diagnosis.xsl"));
                    Patient patient = model.getPatient();
                    String ersteDiagnose = model.getPatient().getErstDiagnose();
                    List<TagesDiagnose> tagesDiagnosen = model
                            .getTagesDiagnosen();
                    ...
                    for (int i = 0; i < tagesDiagnosen.size(); i++) {
                        TagesDiagnose td = (TagesDiagnose) tagesDiagnosen
                                .get(i);
                        if ((td.getText() != null)
                                && (td.getText().length() > 0)) {
                            String[] diag = new String[2];
                            if (td.getDatum() != null)
                                diag[0] = MediknightUtilities.formatDate(td
                                        .getDatum());
                            else
                                diag[0] = "";
                            diag[1] = td.getText();
                            fop.addTagToFather("Tagesdiagnose", "",
                                    "TagesDiagnosen");
                     ...
                     * 
                     * Patient patient = model.getPatient(); String erstDiagnose
                     * = model.getPatient().getErstDiagnose(); List
                     ...
                     * 
                     * dProvider.putData("DAUER", erstDiagnose); List printList
                     * = new ArrayList();
                     * 
                     * for (int i = 0; i < tagesDiagnosen.size(); i++) {
                     * TagesDiagnose td = (TagesDiagnose) tagesDiagnosen.get(i);
                     * if ((td.getText() != null) && (td.getText().length() >
                     * 0)) { String[] diag = new String[2]; if (td.getDatum() !=
                     * null) diag[0] = MediknightUtilities.formatDate(
                     * td.getDatum()); else diag[0] = ""; diag[1] =
                     * td.getText(); printList.add(diag); } } String[][]
                     * printArray = (String[][]) printList.toArray(new
                     * String[0][2]); dProvider.putData("DIAGNOSEN",
                     * printArray);
                     * 
                     * TemplatePrinter tp = new TemplatePrinter(
                     * prop.getProperty("style"), prop.getProperty("header"),
                     * prop.getProperty("diagnosis.content"),
                     * prop.getProperty("footer"), new String[] {
                     * prop.getProperty("frame") }, prop.getProperty("page"),
                     * dProvider);
                     * 
                     * try { tp.print(); } catch (Exception e) { new
                     * ErrorDisplay( e, "Fehler beim Ausdruck!", "Drucken...",
    ...
    public void commit() {
        ...
        try {
             ...
             * for (int i = 0; i < diagnosen.length; i++) { diagnosen[i].save();
             * }
             */
...
//            }
    ...
    private void commit(Component component, LockingInfo.Data data) throws SQLException {
