...
public class MainFrame extends JFrame implements TraceConstants {
	...
	public final static String PROPERTY_FILENAME =
		"de/bo/mediknight/resources/" + MEDIKNIGHT_PROPERTIES;
	...
	private void createUI() {
		...
		subtitle =
			new JLabel(NAME + " " + VERSION + " - Abrechnungsprogramm für Heilpraktiker");
		...
		searchButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			KeyStroke.getKeyStroke(KEYSEARCH),
			"search");
		...
		quitButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			KeyStroke.getKeyStroke(KEYQUIT),
			"quitApplication");
		...
		detailsButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			KeyStroke.getKeyStroke(KEYDETAILS),
			"patientDetails");
		...
		diagnosisButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			KeyStroke.getKeyStroke(KEYDIAGNOSIS),
			"diagnose");
		...
		/*        medicationButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KEYMEDICATION), "verordnung");
		        medicationButton.getActionMap().put("verordnung", new AbstractAction() {
		            public void actionPerformed(ActionEvent e) {
		                medication();
		            }
		        });
		*/
		...
		/*        billButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KEYBILL), "rechnung");
		        billButton.getActionMap().put("rechnung", new AbstractAction() {
		            public void actionPerformed(ActionEvent e) {
		                bill();
		            }
		        });
		*/
		...
		finishButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			KeyStroke.getKeyStroke(KEYFINISH),
			"zurueck");
		...
		aboutButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
			KeyStroke.getKeyStroke(KEYABOUT),
			"about");
	...
	/**
	 * Sets the bill pane as the current pane.
	 * This methods sets the title too.
	 */
	...
	public void reload() {
		/** @todo: HACK!
		 *  Find a better way to suppress committing.
		 */
		...
		switch (action) {
			...
			case CREATE_MACRO :
				/** @todo At the moment it is not possible to return to the create
				    macro function since the information about selected bill entries
				    is lost. */
				bill();
				break;
	...
	/**
	 * This method controls the navigator panel.
	 * Buttons are identified by name.
	 */
	 ...
	 *
	 * @exception SQLException on db error
	 */
	...
	public void setPatientToNavigator(Patient patient) {
		if (patient != null)
			if (patient != null) {
				...
				String date = MediknightUtilities.formatDate(patient.getGeburtsDatum());
				String text =
					"<html><font face=tahoma size=2><b>"
						+ anrede
						+ ((anrede.equals("")) ? "" : "<br>")
						+ firstName
						+ ((firstName.equals("")) ? "" : "<br>")
				...
				patientLabel.setToolTipText(
					((anrede + firstName + lastName + date).equals("")) ? null : text);
			} else {
	...
	public static void initDB() throws Exception {
		...
		// next properties are only set by hypersonic
		String dbName = properties.getProperty("jdbc.db.name");
		String dbHost = properties.getProperty("hsql.server.name");
		String port = properties.getProperty("hsql.server.port");

		if (jdbcURL.indexOf("mysql") >= 0) {
			// If MySQL all connection info except user and password
			// are included in jdbcURL
			runMode = CLIENT;
		} else if (jdbcURL.startsWith("jdbc:hsqldb:")) {
			String s = properties.getProperty("hsql.server.run");
			if (Boolean.valueOf(s).booleanValue()) {
				...
				Server.main(
					new String[] {
						"-port",
						properties.getProperty("hsql.server.port"),
						"-database",
						properties.getProperty("jdbc.db.name"),
						"-silent",
						properties.getProperty("hsql.server.silent"),
						"-trace",
						properties.getProperty("hsql.server.trace")});
				System.exit(0); // Paranoia!
			} else {
				// Hypersonic client or standalone
				runMode = (dbHost != null) ? CLIENT : STANDALONE;
			}
			...
			if (dbName != null) {
				if (runMode == STANDALONE) {
					...
					jdbcURL += home + File.separator + dbName;
				} else {
					jdbcURL += "hsql://" + dbHost;
					if (port != null)
						jdbcURL += ":" + port;
			...
			String msg = "Database specified by jdbc.url.name not supported.";
		...
		//Datastore.current.setAutoCommit(true);
	}

	public static void storeProperties(Properties prop)
		throws FileNotFoundException, IOException {
		System.err.println("Storing properties...");
		properties = prop;
	...
	public static void initTracer() {
		...
		if (logPath == null || logPath.length() == 0) {
			...
			LogWriter writer =
				new LogWriter(
					logPath,
					Long.parseLong(logMaxLength),
					Integer.parseInt(logMaxCycle));
		...
		if (url != null) {
			...
			try {
				...
				if (spos > 0) {
					...
					location =
						new File(ud + File.separator + cp.substring(0, spos)).getCanonicalFile();
	...
	private class GrayedPane extends JComponent {
		...
		public void paint(Graphics g) {
			...
			if (bufferImage == null
				|| bufferImage.getHeight(this) != r.height
			...
			String[] text =
				{
					...
					"",
					"Die Daten werden von einem",
					"anderen Benutzer geändert!" };
	...
	/**
	 * This universal Mouse(Motion)Listener eats all MouseEvents
	 * expect those are not under the "blue panel".
	 */
	private class MyMouseInputAdapter extends MouseInputAdapter {
		...
		public void mouseMoved(MouseEvent e) {
			...
			glasspane.setCursor(
				Cursor.getPredefinedCursor(
					(pc.x < 0) ? Cursor.DEFAULT_CURSOR : Cursor.WAIT_CURSOR));
		}
		...
		private void redispatchMouseEvent(MouseEvent e) {
			// Mouse position relativ to the application content (e.g. the glass pane)
			Point p = e.getPoint();
			...
			if (pc.x < 0) { // we are in navigator, do not eat!
				...
				// Gets component
				Component comp = SwingUtilities.getDeepestComponentAt(navigator, pn.x, pn.y);
