...
public class MacroPresenter implements Presenter, Commitable {
    ...
    void deleteMacro() {
        ...
        if (macro != null) {
            int r = JOptionPane.showConfirmDialog(view, "Baustein "
                    + macro.getAbk() + " wirklich löschen?",
                    "Baustein löschen", JOptionPane.YES_NO_OPTION,
    ...
    public void addMacro() {
        ...
        if (macro != null) {
            ...
            BillEntry[] newEntries = new BillEntry[entries.length
                    + macroEntries.length];

            System.arraycopy(entries, 0, newEntries, 0, entries.length);
            System.arraycopy(macroEntries, 0, newEntries, entries.length,
                    macroEntries.length);
