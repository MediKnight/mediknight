...
/**
 * <code>UndoHandler</code> is a new interface that combines the former
 * <code>UndoManager</code> and <code>UndoMediator</code> interfaces into one.
 * It basically has the same functionality, although -- of course -- the
 * interaction between those two interfaces has been removed.#
 *
 * The purpose of this interface is to enable widgets to act as "Undo Handlers"
 * (hence the name) for groups of other widgets. For this purpose, a widget
 * implementing this class must have a unique name; each <code>Mutable</code>
 * widget knows the name of it's responsible <code>UndoHandler</code> and can
 * use this to fire <code>MutableChangeEvent</code>s to it's
 * <code>UndoHandler</code>, which -- in turn -- can use the information
 * contained therein to act accordingly.
 *
 * <em>Hints:</em><ul>
 * <li>When reverting a radio button, all buttons in this button's buttongroup
 * should be reverted. Unfortunately, the <code>ButtonModel</code> interface
 * does not provide a method for finding out which group a button belongs to;
 * the <code>DefaultButtonModel</code>, though, does.
 *
...
public interface UndoHandler extends MutableChangeListener {
     ...
     * if there are currently no widgets awaiting undo.
     *
     * @param b the new undo backend, which must be an instance of
     * <code>UndoBackend</code>.
     * @throws CurrentlyProhibitedException if invoked while there still are
     * Undo candidates.
     *
     * @since 1.0
     */
     ...
     * widgets awaiting possible undo (Undo candidates).
     *
     * @since 1.0
     */
    ...
    /**
     * Set whether Undo is currently possible based on whether there are
     * currently any <code>Mutable</code> objects registered for Undo or not.
     * More precisely, Undo will be possible iff there are any such objects
     * present currently.
     *
     * @since 1.0
     */
     ...
     * What this means specifically is left to the implementing class; usually,
     * one'll want to move the object to the top of an <code>UndoStack</code>
     * of objects awaiting possible undoing, but that's just one thing that can
     * be done, and more (or less) elaborate setups are certainly possible.
     *
     ...
     * undoing. See <code>registerUndoCandidate</code> above for more.
     *
     ...
     * the list.
     *
     * @return the next candidate for Undo, or null if no such candidate is
     * present.
     *
     * @since 1.0
     * @see Mutable
