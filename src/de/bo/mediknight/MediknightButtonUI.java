...
/**
 * A new LAF for the <code>JButton</code> which uses a gradient to paint
 * the button's beackground and marks default buttons with a CR icon.
 *
...
public class MediknightButtonUI extends MetalButtonUI implements SwingConstants {
     ...
     * a gradient.  The default button is additionally marked with a CR-icon.
     * This method is postly copied from <code>MetalButtonUI</code> with some
     * changes to support the CR-icon.
     */
    public void paint(Graphics g, JComponent c) {
        ...
        String text = SwingUtilities.layoutCompoundLabel(
            ...
            hasDefaultIcon ? CENTER : b.getVerticalTextPosition(),
            hasDefaultIcon ? LEADING : b.getHorizontalTextPosition(),
        ...
        if (b.isContentAreaFilled()) {
            Color color = ((JButton)b).isDefaultButton() ?
        ...
        if (b.isFocusPainted() && b.hasFocus()) {
            ...
            if (hasDefaultIcon) textRect.width += 16;
    ...
    /**
     * Paints the gradient.
     * @param g the <code>Graphcis</code> object to paint to
     * @param color base color, gradient goes from brighter() to darker().
     * @param w the component's width
     * @param h the component's height
     */
    public static void paintGradient(Graphics2D g, Color color, int w, int h) {
        g.setPaint(new GradientPaint(
            0f, 0f, brighter(color, 0.8),
            0f, h * 0.4f, color,
            false));
        g.fillRect(0, 0, w, h / 2);
        g.setPaint(new GradientPaint(
            0f, h * 0.6f, color,
            0f, (float)h, darker(color, 0.8),
            false));
        g.fillRect(0, h / 2, w, h / 2);
    }

    /**
     * Returns a brighter color.  Same algorithms as
     * <code>Color.brighter()</code> but with a variable factor.
     */
    static final Color brighter(Color color, double factor) {
        int r = color.getRed();
    ...
    /**
     * Returns a darker color.  Same algorithms as
     * <code>Color.darker()</code> but with a variable factor.
     */
    static final Color darker(Color color, double factor) {
