...
/**
 * A simple tiny layout manager aligning components in a grid with the following constraints
 * 
 * <pre>
 * x, y, align
 * </pre>
 * 
 * where x denots horizontal layout, y vertical layout and align the horizontal alignment. All values are integers. -1 means space filling, 0 means the
 * component's preferred size and a number larger then 0 is a logical size. The alignment can be C, N, E, W, S, NW...
 *
...
public class FlexGridLayout implements LayoutManager2 {
    ...
    /**
     * Constraint class for FlexGridLayout. BLC seems to stand for "... layout constraint", but I have no idea what the B means. Maybe "Baltic Online", but I
     * doubt that.
     *
	 ...
	 *
	 * @param constraint
	 *            a string specifying the constraints
	 *
	 * @since 1.0
	 */
	BLC( final String constraint ) {
	    final StringTokenizer st = new StringTokenizer( constraint, "," );
	    if( st.hasMoreTokens() ) {
	 ...
	 *
	 * @param the
	 *            string containing the alignment specification
	 * @exception IllegalArgumentException
	 *                if the string does not specify a valid alignment
	 *
	 * @since 1.0
	 */
    ...
    /**
     * Create a new <code>FlexGridLayout</code> with one row and an arbitrary number of columns, setting both <code>hgap</code> and <code>vgap</code> to 0.
     *
     * @since 1.0
     */
    ...
    /**
     * Create a new <code>FlexGridLayout</code> with the specified number of rows and columns, setting both <code>hgap</code> and <code>vgap</code> to 0.
     *
     * @param rows
     *            the number of rows for the new <code>FlexGridLayout</code>
     * @param cols
     *            the number of columns for the new <code>FlexGridLayout</code>
     *
     * @since 1.0
     */
    public FlexGridLayout( final int rows, final int cols ) {
	this( rows, cols, 0, 0 );
    }


    /**
     * Create a new <code>FlexGridLayout</code> with the specified number of rows and columns and the specified values for <code>hgap</code> and
     * <code>hgap</code>
     *
     * @param rows
     *            the number of rows for the new <code>FlexGridLayout</code>
     ...
     *            the number of columns for the new <code>FlexGridLayout</code>
     * @param hgap
     *            the horizontal gap for the new <code>FlexGridLayout</code>
     * @param vgap
     *            the vertical gap for the new <code>FlexGridLayout</code>
     *
     * @since 1.0
     */
    public FlexGridLayout( final int rows, final int cols, final int hgap, final int vgap ) {
	if( rows == 0 && cols == 0 ) {
    ...
    /**
     * Adds the specified component to the layout, using the specified constraint object. Ignored by this layout manager.
     *
     * @param name
     *            the component name
     * @param comp
    ...
    /**
     * Invalidates the layout, indicating the if the layout manager has cached information, it should be discarded.
     *
     * @param target
	...
	synchronized( target.getTreeLock() ) {
	    ...
	    final int widths[] = new int[ncomponents];
	    final int heights[] = new int[ncomponents];
	    int nrows = rows;
	    int ncols = cols;
		...
		final BLC blc = constraints.get( c );
		if( blc == null || blc.x == 0 ) {
	    ...
	    final Dimension dt = target.getSize();
	    dt.width -= insets.left + insets.right + (ncols - 1) * hgap;
	    dt.height -= insets.top + insets.bottom + (nrows - 1) * vgap;

	    /*
	     * WARNING!! This code fixes a problem with unreported insets when a JPanel is embedded within a JViewport (used by JScrollPane). The JPanel does
	     * NOT report the insets created by the JScrollPane and the dimensions used are therefore always too large for the display area.
	     */
	    ...
	    final int minColWidth[] = new int[ncols];
	    final int minRowHeight[] = new int[nrows];
	    final boolean colSpace[] = new boolean[ncols];
	    final boolean rowSpace[] = new boolean[nrows];
		...
		final int heightFill = (dt.height - minUsedHeight) / noRowFill;

		index = 0;
		for( int row = 0; row < nrows; row++ ) {

		    final int startHeight = minRowHeight[row];
		    for( int column = 0; column < ncols; column++, index++ ) {
			if( index >= ncomponents ) {
			    continue;
			}
		...
		for( int column = 0; column < ncols; x += minColWidth[column] + vgap, column++, index++ ) {
		    ...
		    final Component c = target.getComponent( index );
		    if( !c.isVisible() ) {
			continue;
		    }
